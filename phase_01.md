...
| 原方案 | 最终方案 |
|--------|---------|
| 策略模板 | **5策略**：MA突破/布林回归/放量突破/ATR通道/动量突破，每策略**5参数**（入场2 + ATR止损 + 止盈% + 最大持仓时间） |
| 原始 | 固定百分比止损 | **ATR倍数止损 + 止盈 + 时间止损**（三重风控） |
| 原始 | 千1.5手续费 | **千1.5 + 0.1%滑点 + 涨跌停检测** |
| 原始 | T+1 以 n//2 硬切 | **按 ts.dt.date 真实交易日分组** |
| 原始 | 无缺口处理 | **has_gap() 检测停牌断裂窗口并跳过** |
